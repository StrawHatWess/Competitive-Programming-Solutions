...
int f2(int i){
	...
	int lft=0,rgt=sz(v2)-1,idx=-1;
	while(lft<=rgt){
		int mid=(lft+rgt)/2;
		if(x-pl[v2[mid]]<l+pr[v2[mid]]){
			idx=mid;
			rgt=mid-1;
		}	
		else lft=mid+1;
	...
	int rmv=l+pr[v2[idx]]-(x-pl[v2[idx]]);
	/*if(idx) ckmin(rmv,pl[v[idx]]-pl[v[idx-1]]);
	else ckmin(rmv,pl[v[idx]]);*/

	int prev=x;
	if(idx) prev-=pl[v2[idx-1]];

	if(prev<l+pr[v2[idx]]){
		rmv=pl[v2[idx]];
		if(idx) rmv-=pl[v2[idx-1]];
	}

	if(v2[idx]+1<M) rmv+=sl[v2[idx]+1];
...
int32_t main() {
    ...
    int mn=0,mx=0,cur=0;
   	...
   	M=sz(vec);
   	FOR(i,0,M) if(vec[i].fi) v.pb(i); else v2.pb(i);
