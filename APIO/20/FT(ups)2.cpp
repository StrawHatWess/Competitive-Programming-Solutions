...
vi createFunTour(int N, int Q) {
	::N=N; if(N==2) return {0,1};

	C=find_centroid();
	decompose();

	int cur=farthest(),prev=-1,first=1;
	while(check()){
		del(cur);
		prev=cur;
		cur=find_nxt(prev);
	...
	if(!first) s[comp[prev]].insert({dC[prev],prev});

	combine();

	if(!first) s[comp[prev]].erase({dC[prev],prev});

	if(first){
		auto it=s[0].end(); it--; 
		cur=(*it).se;
	}
	else if(!(comp[prev]==comp[cur] && dC[prev]<dC[cur]))
		cur=find_nxt(prev);
