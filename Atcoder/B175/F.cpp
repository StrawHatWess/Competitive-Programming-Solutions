...
int32_t main() {
	...
	//edges cases: empty strings - similar strings in input	
	...
	FOR(i,0,N){
		...
		q.push({dist[{a[i],0}],{a[i],0}}); q.push({dist[{a[i],1}],{a[i],1}});
	}

	int ans=INF;
	while(!q.empty()){
		str s=q.top().se.fi; 
		int side=q.top().se.se,d=q.top().fi;
		q.pop();
		int dd=INF; if(dist.count({s,side})) dd=dist[{s,side}];
		if(d>dd) continue;
		if(pal(s)){ans=min(ans,d); continue;}

		FOR(i,0,N){
			if(!can(s,side,a[i])) continue;
			str ss=combine(s,side,a[i]);
			int sidep=(sz(s)<sz(a[i]));
			//cout << s << ' ' << side << ' ' << a[i] << ' ' << sidep << endl;
			if(side==1) sidep=1-sidep;
			//cout << s << ' ' << side << ' ' << a[i] << ' ' << sidep << endl;
